...
parser.add_argument("--fp8-recipe", type=str, default="tensorwise", choices=[
                    "rowwise", "tensorwise"], help="FP8 scaling recipe: tensorwise (faster, recommended) or rowwise (more accurate but slower)")
# Model architecture
...
parser.add_argument("--max-seq-len", type=int,
                    default=2048, help="max context length")
...
# AttnRes / Engram architecture extensions
parser.add_argument("--use-attn-res", action="store_true",
                    help="enable Block Attention Residuals")
parser.add_argument("--attn-res-num-blocks", type=int,
                    default=4, help="number of AttnRes blocks")
parser.add_argument("--use-engram", action="store_true",
                    help="enable Engram conditional n-gram memory")
parser.add_argument("--engram-table-size", type=int, default=131072,
...
parser.add_argument("--engram-param-ratio", type=float, default=0.15,
                    help="fraction of total params allocated to Engram tables")
...
parser.add_argument("--warmdown-ratio", type=float,
                    default=0.65, help="ratio of iterations for LR warmdown")
...
parser.add_argument("--eval-tokens", type=int, default=80 *
                    524288, help="number of tokens to evaluate val loss on")
parser.add_argument("--core-metric-every", type=int, default=2000,
                    help="evaluate CORE metric every N steps (-1 = disable)")
parser.add_argument("--core-metric-max-per-task", type=int,
                    default=500, help="examples per task for CORE metric")
...
def build_model_meta(depth):
    ...
    model_dim = ((base_dim + args.head_dim - 1) //
                 args.head_dim) * args.head_dim
    num_heads = model_dim // args.head_dim
    engram_layers = [int(x) for x in args.engram_inject_layers.split(
        ',') if x.strip()] if args.engram_inject_layers else []
    config = GPTConfig(
...
if args.fp8:
    if device_type != "cuda":
        ...
        num_linear = sum(1 for m in model.modules()
                         if isinstance(m, nn.Linear))
        convert_to_float8_training(
            model, config=fp8_config, module_filter_fn=fp8_module_filter)
        num_fp8 = sum(1 for m in model.modules()
                      if 'Float8' in type(m).__name__)
...
# original, uncompiled model, for saving raw model state_dict and for inference/evaluation (because the shapes may change shape)
orig_model = model
...
def get_scaling_params(m):
    ...
    scaling_params = params_counts['transformer_matrices'] + \
...
weight_decay_scaled = args.weight_decay * \
    math.sqrt(total_batch_size / B_REF) * (D_REF / target_tokens)
...
def build_val_loader(): return tokenizing_distributed_data_loader_bos_bestfit(
...
if args.num_iterations > 0:
    ...
    num_iterations = round(args.target_flops /
                           (num_flops_per_token * total_batch_size))
    print0(
...
tokens_per_fwdbwd = args.device_batch_size * \
    args.max_seq_len  # tokens per iteration for a single rank
world_tokens_per_fwdbwd = tokens_per_fwdbwd * \
    ddp_world_size  # total tokens per iteration for all ranks
...
while True:
    ...
    if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        eval_steps = args.eval_tokens // (args.device_batch_size *
                                          args.max_seq_len * ddp_world_size)
    ...
    smooth_train_loss = ema_beta * smooth_train_loss + \
        (1 - ema_beta) * train_loss_f  # EMA the training loss
    debiased_smooth_loss = smooth_train_loss / \
        (1 - ema_beta**(step + 1))  # debias the EMA
    ...
    first_step_of_run = (step == 0) or (
        resuming and step == args.resume_from_step)
    step += 1
