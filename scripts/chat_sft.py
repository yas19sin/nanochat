...
parser.add_argument("--mixture", type=str, default="darija", choices=["darija", "legacy"], help="SFT data mixture: darija instruct datasets or the legacy SmolTalk/MMLU/GSM8K blend")
...
if args.mixture == "legacy":
    ...
    print0(f"Training mixture: legacy blend | MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs}")
else:
    from tasks.darija_instruct import DarijaInstruct, DarijaTulu

    train_tasks = [
        DarijaInstruct(split="train"),
        DarijaTulu(split="train"),
    ]
    val_dataset = TaskMixture([
        DarijaInstruct(split="test"),
        DarijaTulu(split="validation"),
    ])
    print0("Training mixture: Darija instruct datasets (Lyte + GemMaroc)")

train_dataset = TaskMixture(train_tasks)
print0(f"Total training rows: {len(train_dataset):,}")
