...
# Darija 1B + Engram — n-gram memory enabled, no AttnRes.
...
#   bash runs/darija_engram.sh        # this script
#   WANDB_RUN=darija_engram screen -L -Logfile runs/darija_engram.log -S engram bash runs/darija_engram.sh
...
# -----------------------------------------------------------------------------
# Base model pretraining — depth=21 (~1.1B trunk + ~137M Engram)
torchrun --standalone --nproc_per_node=8 -m scripts.base_train \
    ...
    --model-tag=d21_engram --run="$WANDB_RUN"
...
echo ""
echo "=== Darija + Engram (1B) complete ==="
